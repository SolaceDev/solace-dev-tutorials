...
const tutorial = ({data, pageContext}) => {
  ...
  const tutorials = data.tableOfContent.edges.filter((edge) => edge.node.frontmatter.layout === "tutorials" && edge.node.frontmatter.visible !== false)
  console.log("Page Headings")
  const headings = node.headings.filter((heading) => heading.depth == 2)
  const features = data.tableOfContent.edges.filter((edge) => edge.node.frontmatter.layout === "features" && edge.node.frontmatter.visible !== false)
  const feedback = node.frontmatter.links.filter((link) => link.label === "feedback")
  ...
  return (
    <Layout hideResources="true">
      ...
      <section id="breadcrumbs">
        <Container>
          <Breadcrumb crumbs={crumbs_clone}
            crumbLabel={
              node.frontmatter.title
            }
            crumbSeparator=" 👉 "/>
        ...
        <Row>
          ...
          <Col xs={12}
            ...
            <TableOfContent features={features}
              ...
              page_title={page_title}
              slug={slug}
              slugRoot={slugRoot}></TableOfContent>
          </Col>
        </Row>
      </Container>
    </Layout>
